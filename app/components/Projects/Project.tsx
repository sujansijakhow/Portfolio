...
const Project = ({
  ...
  return (
    <Reveal initialX={-50} delay={0.5}>
      <motion.div onMouseMove={handleMouseMove} onMouseLeave={handleMouseLeave} className='card flex flex-col items-stretch w-full max-w-[428.4px]
        p-5 md:p-[18px] gap-[30px]' style={{
          transformStyle: "preserve-3d",
          rotateX: xRotation,
          rotateY: yRotation,
        }}>
          <Image src={thumbnail} alt={`Thumbnail for ${title}`} width={392} height={230}
            className='w-full h-[230px] object-cover' style={{
              transform: "translateZ(100px)",
            }} />
            <div className='flex flex-col gap-[11px]'>
              <div className='flex justify-between items-center'>
                <h3 className='font-semibold text-xl md:text-[22px]'>{title}</h3>
                <a href={link.url} className='flex px-[5px] py-[3px] gap-1 bg-[#b9b9b9]
                  bg-opacity-[24%] rounded text-[14px]'>
                    <span className='hidden md:block'>{link.label}</span>
                    <img src="/link_arrow.svg" alt="Link arrow" className='block dark:hidden' />
                    <img src="/link_arrow_dark.svg" alt="Link Dark arrow" className='hidden dark:block' />
                  </a>
              </div>
              <p className='line-clamp-2 text-sm md:text-base'>{description}</p>
              <div className='flex flex-row gap-[11px]'>
                {languageIcons.map((icon, iconId) => (
                    <img src={icon} alt='Language Icons' key={iconId} />
                  ))
                }
              </div>
            </div>
